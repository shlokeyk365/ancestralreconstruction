...
# 4. Plotting: 2 environmental plots + 6 species occurrence plots
par(mfrow = c(2, 4), mar = c(0, 0, 0, 0))
# Plot temperature (bio_1)
plot(wclim_df$x, wclim_df$y, pch = 22, cex = 0.5,
     bg = gray.colors(100, start=1, end=0)[as.numeric(cut(wclim_df$bio_1, breaks=100))],
     col = gray.colors(100, start=1, end=0)[as.numeric(cut(wclim_df$bio_1, breaks=100))],
     axes = FALSE, xlab = "", ylab = "", main = "Temperature (bio_1)")
# Plot precipitation (bio_12)
plot(wclim_df$x, wclim_df$y, pch = 22, cex = 0.5,
     bg = gray.colors(100, start=1, end=0)[as.numeric(cut(wclim_df$bio_12, breaks=100))],
     col = gray.colors(100, start=1, end=0)[as.numeric(cut(wclim_df$bio_12, breaks=100))],
     axes = FALSE, xlab = "", ylab = "", main = "Precipitation (bio_12)")
# Plot 6 species occurrence maps
for (i in 1:6) {
  plot(wclim_df$x, wclim_df$y, pch = 22, cex = 0.5,
       bg = gray.colors(100, start=1, end=0)[as.numeric(cut(occurrence_probs[, i], breaks=100))],
       col = gray.colors(100, start=1, end=0)[as.numeric(cut(occurrence_probs[, i], breaks=100))],
       axes = FALSE, xlab = "", ylab = "", main = paste("Species", i))
} 